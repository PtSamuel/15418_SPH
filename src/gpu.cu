...
// SCAN EQUIVALENCE
__global__ void find_dividers(int n, Particle *particles) {
    int blockprev = -1;
...
__global__ void compute_x_dot(int n, SwapStatus status) {
    ...
    if(status == SWAP_DEFAULT) {
        ...
        // LEAPFROG
        updated_x_dot.vel.x = cur.vel.x + acc.x * params.dt;
        updated_x_dot.vel.y = cur.vel.y + acc.x * params.dt;
...
__global__ void step_ahead(int n, Particle *particles, Particle *update) {
    ...
    // LEAPFROG
    cur.pos.x += cur.vel.x * params.dt * 10;
    cur.pos.y += cur.vel.y * params.dt * 10;
    // cur.vel.x += cur.acc.x * params.dt * 0.5;
    // cur.vel.y += cur.acc.y * params.dt * 0.5;
